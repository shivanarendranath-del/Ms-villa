  ...
  function renderModal(){
    ...
    wrap.querySelectorAll(".de-photo-file").forEach(fileInput=>{
      fileInput.onchange = async ()=>{
        ...
        try{
          draft[i].photo = await readAndCompressImage(f);
          renderModal();
...
function renderRoommates(){
  ...
  const cards = state.members.map(m=>`
    <div class="roommate-card">
      ${m.photo ? `<img class="roommate-photo" src="${m.photo}">` : `<div class="roommate-photo-empty">${(m.name||"?")[0]}</div>`}
...
function openEditRoommateModal(username){
  const member = state.members.find(m=>m.username===username);
  if(!member) return;
  const wrap = document.createElement("div");
  wrap.className = "modal-bg";
  let photoData = member.photo || null;
  wrap.innerHTML = `
    <div class="modal">
  ...
  wrap.querySelector("#rm-save").onclick = async ()=>{
    ...
    const prevName = member.name, prevEdu = member.education, prevPhoto = member.photo;
    member.name = name;
    ...
    if(!ok){
      member.name = prevName; member.education = prevEdu; member.photo = prevPhoto;
...
function renderSettings(){
  ...
  if(me.admin){
    ...
    $("#send-announce").onclick = async ()=>{
      ...
      await notifyMembers(title, body, { excludeUsername: null });
      btn.disabled = false; btn.textContent = "Send to Everyone";
      msg.style.display="block"; msg.textContent="Announcement sent.";
...
(async function init(){
  ...
  if(remembered && state.members.some(m=>m.username===remembered.username)){
    ...
    const validViews = ["home","room","instructions","settings","changepass","duty","expenses","dailyExpenses","rent","complaints","meetings","roommates"];
